...
Three networks are measured: **Ethereum**'s consensus-layer discv5 (55,095 nodes),
**Logos Messaging** (formerly Waku, n=56), and **Logos Storage** (formerly Codex, n=8), which
runs a discv5-derived DHT using libp2p Signed Peer Records instead of EIP-778 records. Ethereum and Logos Messaging carry the quantitative
results; Logos Storage is used only as an existence proof about the record container.
...
| `data/crawl-waku*.csv` | Logos Messaging crawls (n=56, ~60,500 observations), a small, homogeneous, near-cap network used as a regime contrast against Ethereum |
...
**Logos Storage source records are also withheld**, for the same reason. Logos Storage publishes libp2p Signed
Peer Records (`spr:`) rather than ENRs, and these likewise embed addresses inside a base64
encoding, they scan clean as plaintext and are not. The Logos Storage figures in the paper are record
...
**Conflict of interest.** Logos Messaging is a Logos project, and Logos is the author's employer.
Those measurements are of a production fleet operated by that employer.
...
| `reclaim.py` | `waku-records.txt` | produced by a Logos Messaging crawl; open an issue for a redacted copy |
...
- **Logos Messaging is n=56 and Logos Storage n=8.** Too small to support percentages as results; used as contrast.
